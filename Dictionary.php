...
class Dictionary
{
     ...
     * has a "word" field and a "score_modifier" field.  This means that your dictionary should probably
     * list a score of -1 for negative words/phrases and a score of 1 for positive words/phrases.
     * @param  String $file_path        Location of the dictionary file on the server.
     * @param  String $inline_delimiter Delimiter that separates a token from another in an entry.  For example, in "A LIE -1", the " " is the $inline_delimiter
     * @param  String $entry_delimiter  Delimiter that separates one dictionary entry from another.
     * @return StringArray              Array representation of the dictionary, containing "word" and "score_modifier" fields.
     */
    public function read_dictionary($file_path, $inline_delimiter, $entry_delimiter)
    {
        ...
        for($i = 0; $i < count($dictionary_array); $i++) {
            ...
            //we only have a single word and a score - AXED -1
            if(count($current_entry) == 2) {
                $dictionary[$i]["word"] = trim($current_entry[0]);
                $dictionary[$i]["score_modifier"] = trim($current_entry[1]);
            } else {
                //current spot has multiple space delimited tokens - A LIE -1
                $build_word = "";
                for($j = 0; $j < count($current_entry); $j++) {
                    if(is_numeric(trim($current_entry[$j]))) {
                        ...
                        $dictionary[$i]["score_modifier"] = trim($current_entry[$j]);
                    } else {
     ...
     * to parse properly due to the use of * characters in tons of entries (AX* -1, BOTHER* -1, etc).  Other
     * dictionaries (I assume) would omit the use of the *, thus they would be able to utilize the regular
     * read_dictionary() function.
     * @param  String $file_path        Location of the dictionary file on the server.
     * @param  String $inline_delimiter Delimiter that separates a token from another in an entry.  For example, in "A LIE -1", the " " is the $inline_delimiter
     * @param  String $entry_delimiter  Delimiter that separates one dictionary entry from another.
     * @param  String $other_delimiter  Optional delimiter that can further delimit entries.  In "BLINDING* -1", the * is the $other_delimiter
     * @return StringArray              Array representation of the dictionary, containing "word" and "score_modifier" fields.
     */
    public function read_LSD_dictionary($file_path, $inline_delimiter, $entry_delimiter, $other_delimiter)
    {
        ...
        for($i = 0; $i < count($dictionary_array); $i++) {
            if(strpos($dictionary_array[$i], $other_delimiter) !== false) {
                ...
                $dictionary[$i]["word"] = trim($current_entry[0]);
                $dictionary[$i]["score_modifier"] = ( (trim($current_entry[1]) == 0) ? -1 : 1);
            } else {
                $current_entry = explode($inline_delimiter, $dictionary_array[$i]);
                //we only have a single word and a score - AXED -1
                if(count($current_entry) == 2) {
